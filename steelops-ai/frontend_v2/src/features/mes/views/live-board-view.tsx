...
export function LiveBoardView() {
  ...
  const load = useCallback(() => {
    mesApi
      ...
      .then(({ data }) => {
        const d = data as {
          columns: Record<string, HeatCard[]>;
  ...
  const hasBoard = Object.keys(columns).length > 0 || Object.keys(counts).length > 0;

  return (
    <PageContainer
      title="Live Board"
      description={`Heats by status for furnace ${furnaceId} — refreshes every 30 seconds.`}
    ...
    >
      <PageExplainer {...PAGE_EXPLAINERS.liveBoard} />
          ...
          <div className="grid gap-3 lg:grid-cols-3 xl:grid-cols-6">
            {COLS.map((col) => {
              ...
              return (
                <SectionCard
                  key={col}
                  title={humanizeKey(col)}
                  description={`${counts[col] ?? 0} heats`}
                >
